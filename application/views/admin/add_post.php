			...
			<div class="card-body">
				<div class="add-new-post">
					<input class="form-control form-control-lg mb-3" type="text" placeholder="Your Post Title"
						   name="post_title" value="<?= $query ? $query->post_title : '' ?>">
					<div id="editor-container" class="add-new-post__editor mb-1"></div>
					<textarea name="post_content" cols="0" rows="0"
							  class="d-none"><?= $query ? $query->post_content : '' ?></textarea>
					<div class="mb-3 mt-3">
					...
					<?php if (isset($query->thumbnail)): ?>
						<div class="mb-3 d-flex justify-content-center">
							<div class="thumbnail-container position-relative">
								<img src="<?= base_url('upload/thumbnail/' . $query->thumbnail) ?>" alt="Thumbnail"
									 class="img-thumbnail shadow-sm" id="thumbnailPreview">
							</div>
						</div>
					<?php endif ?>
				</div>
						...
						<?php if ($categories and $categories !== NULL): ?>
							<?php foreach ($categories as $cat): ?>
								<div class="custom-control custom-radio mb-1">
									<input type="radio" name="post_cat_id" class="custom-control-input"
										   id="<?= $cat->id ?>"
										   value="<?= $cat->id ?>" <?= $query ? ($query->post_cat_id == $cat->id ? 'checked' : '') : ''; ?>>
									<label class="custom-control-label"
										   for="<?= $cat->id ?>"><?= $cat->cat_title ?></label>
								</div>
							<?php endforeach ?>
						<?php else: ?>
